...
public class FloorMaker : MonoBehaviour 
{
	...
	public static int gTileCount;//Declare a private integer called myCounter that starts at 0; 		// count how many floor tiles this FloorMaker has instantiated
	public Transform floorPrefab;//Declare a public Transform called floorPrefab, assign the prefab in inspector;
	public Transform floorMakerPrefab;//Declare a public Transform called floorMakerPrefab, assign the prefab in inspector; 

	void Update () 
	{
		if (gTileCount < 500)//If counter is less than 50, then:
							 //Generate a random number from 0.0f to 1.0f;
		{
			...
			else if (randNum >= 0.25f && randNum <= 0.5f)//Else if number is 0.25f-0.5f, then rotate myself -90 degrees on Z axis;
			{
				transform.Rotate(0f, 0f, -90f);
			}
